...
#include "exploit/voucher_swap/kernel_slide.h"
#include "exploit/voucher_swap/kernel_memory.h"
...
//    char* cmp = "0xfeedface";
//
//    pread(fd, buf1, sizeof(buf1), k);
//
//    while (strcmp(buf1, cmp) != 0) {
//        k+=1;
//        pread(fd, buf1, sizeof(buf1), k);
//        printf("%s", buf1);
//    }
//
//    pread(fd, buf1, sizeof(buf1), 435);
//
//
...
//
//        while (!ref){
//            ref = XREF64(Kernel, base, end, to+k);
//            printf("still trying....");
//            k+=1;
//        }
...
addr_t Find_trustcache(void) {
    ...
    if (!val) {
        ...
        if (!ref) {
            ref = Find_strref("\"loadable trust cache buffer too small (%ld) for entries claimed (%d)\"", 0, 0, false);
        }
        
        //Cycle through higher values of n
        
//        for (int j = 0; j < 100; j++){
//            for( int k = 0; k < 2; k++){
//                if (!ref){
//                    ref = Find_strref("\"loadable trust cache buffer too small (%ld) for entries claimed (%d)\"", j, k, false);
//                    printf("testing ref at %i, %i\n", j, k);
//                } else {
//                    printf("found ref!\n");
//                    printf("j = %i, k = %i\n", j, k);
//                }
...
//
//        while (!val){
//            val = Calc64(Kernel, ref-12*4, ref-12*4+12, 8+k);
//            k+=1;
//            if (val) break;
//            if (k > 16000) break;
//        }
...
//
//        while (!val){
//            val = Calc64(Kernel, ref-12*4, ref-12*4+12, 8+j);
//            if (val) break;
//            j+=1;
//        }
...
addr_t Find_OSBoolean_True() {
    ...
    return kernel_read64(val + KernDumpBase + kernel_slide);
    //return KernelRead_64bits(val + KernDumpBase + kernel_slide);
}
...
addr_t Find_kernel_map() {
    ...
    return (*(uint64_t *)(Kernel + val)) ? *(uint64_t *)(Kernel + val) : val + KernDumpBase + kernel_slide;
}
